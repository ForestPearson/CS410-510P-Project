...
class reportHandler:
  ...
  def addService(self, serviceName, cost):
    ...
    if error == -1:
        ...
        print("Cost: number greater than or equal to zero.")
        return -1
  ...
  def displayServices(self, serviceCode=0):
    ...
    # Cannot open services.JSON
    if reports == -1:
      print("\nNo services saved.")
      return -1

    # Iterate through list of services
    # Print all if no serviceCode provided
    count = 0
    ...
    if count == 0:
      print("\nNo match found. Please supply a valid service code.")
      return 0

    return 1

  # Delete service associated with passed in service code
  # or delete all services by default
  def delService(self, serviceCode=0):

    flag = self.rm.delService(self.rm, serviceCode)

    if flag == -2:
      print("No services saved. Cannot delete services.")
    elif flag == -1:
      print("All services deleted.") 
    elif flag == 0:
      print("No service with service code #", serviceCode, "found.")
    else:
